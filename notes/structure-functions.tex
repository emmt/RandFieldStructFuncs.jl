...
\abstract{This note proposes a fast algorithm to compute the empirical
...
The structure function of the centered random field $\phi(r)$ is defined by:
\begin{equation}
  \label{eq:D-phi}
...
Developping the expression of the field structure function and since $\phi(r)$
is centered $\forall r$, i.e.\ $\langle\phi(r)\rangle = 0$, yields:
\begin{align}
  D_{\phi}(\Delta r)
...
Introducing the normalized support function\footnote{so-called normalized
  \emph{pupil function} in optics} $S(r)$ such that for any function $f(r)$:
\begin{align}
  \label{eq:S-def}
